...
from src.job.test_elff_preprocess.event import PreprocessEvent

import json

class elff_job(StreamProcessor):
    def __init__(
        self,
        job_name: str,
        **kwargs
    ...
    def process(self):
        ...
        ).process(PreprocessEvent(), output_type=Types.STRING()).set_parallelism(4)\
            .map(redis_sink, output_type= Types.STRING()).set_parallelism(2)
...
if __name__ == "__main__":
    stream = elff_job(job_name=Path(__file__).stem)
    stream.execute()
